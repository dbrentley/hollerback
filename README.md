...
**Nothing is named.** A session identifies itself as
`<host>:<project-dir>#<tag>`, where the tag comes from its own Claude Code session
id — so two sessions sharing a working directory, doing unrelated work, are still
distinct. It then says what it *does* at runtime with `announce()`. So this installs once per machine
...
by starting in a directory — it identifies itself as `<host>:<project-dir>#<tag>`
and `discover()` finds it. The tag comes from the session itself, so two sessions
in one directory never share an inbox, and an `announce()` cannot overwrite
